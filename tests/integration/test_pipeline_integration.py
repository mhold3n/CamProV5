...
def test_pipeline_integration_component_validation(tmp_path):
    ...
    optimal_profiles = result['optimal_profiles']  # noqa: F841
...
def test_pipeline_integration_data_consistency(tmp_path):
    ...
    profiles = optimal_profiles  # Direct access, no nesting
    ...
    # Gear ratio should be consistent (check if key exists)
    if 'gear_ratio' in profiles:
    	assert profiles['gear_ratio'] == params['gearRatio']
    # Note: gear_ratio key may not exist in current implementation
