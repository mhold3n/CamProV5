...
def compare_results(result1, result2, name1, name2):
    ...
    if isinstance(eff1, list):
        eff1 = np.mean(eff1)
    if isinstance(eff2, list):
        eff2 = np.mean(eff2)
    ...
    if significant_diff:
        print("  ✅ Results are significantly different - parameters are being used!")
    else:
        print("  ❌ Results are nearly identical - possible fallback or caching issue")
