...
class BatchProcessor(
    ...
    suspend fun processBatch(
        ...
        currentBatchJob = CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                logger.info("Batch processing completed: ${results.count { it.status == BatchResultStatus.Success }} successful, ${results.count { it.status == BatchResultStatus.Failed }} failed")
    ...
    suspend fun exportBatchResults(
        outputPath: Path,
        format: ResultExporter.ExportFormat = ResultExporter.ExportFormat.JSON
    ): Boolean {
        return try {
            val currentResults = _results.value
