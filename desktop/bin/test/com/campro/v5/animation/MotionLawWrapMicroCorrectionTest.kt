    ...
    fun centered_wrap_witnesses_and_means_are_within_tolerance() {
        ...
        for (profile in profiles) {
            for (step in steps) {
                ...
                val accel = DoubleArray(n) { m.samples[it].aMmPerOmega2 }
                ...
                val a360 = accel[i2] + ratio * (accel[i2] - accel[i1])
                val vMax = vs.maxOf { kotlin.math.abs(it) }.coerceAtLeast(1.0)
                val aMax = accel.maxOf { kotlin.math.abs(it) }.coerceAtLeast(1.0)
                ...
                assertEquals(a360, accel[0], ta, "Wrap accel mismatch (extrapolated a)")
            }
        }
    }
}
