...
class MotionLawContinuityTest {
    ...
    fun periodicity_wrap_and_dwell_zeros_for_all_profiles() {
        ...
        for (profile in profiles) {
            ...
            val accel = DoubleArray(n) { motion.samples[it].aMmPerOmega2 }
            ...
            ): Boolean = th >= start - 1e-9 && th < end - 1e-9
            for (i in 0 until n) {
                ...
                if (inRange(th, 0.0, p.dwellTdcDeg) || inRange(th, bdcStart, bdcEnd)) {
                    ...
                    assertEquals(0.0, accel[i], 1e-6, "a should be ~zero in dwells @θ=$th (profile=$profile)")
                }
            }
            ...
            val jerk =
                DoubleArray(n) {
                    val im1 = at(it - 1)
                    val ip1 = at(it + 1)
                    val daPerDeg = (accel[ip1] - accel[im1]) / (2.0 * step)
                    daPerDeg * deg2rad // per-omega^3
                }
            ...
            run {
                ...
                MotionLawAssertions.assertWrapContinuityExtrapolated360(theta, xs, vs, accel, tol)
            }
        }
    }
