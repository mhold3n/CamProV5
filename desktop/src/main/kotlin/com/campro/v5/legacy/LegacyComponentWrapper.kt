...
object LegacyComponentWrapper {
     ...
     */
    inline fun <T> withMotionLawEngine(action: () -> T): T? {
        return if (FeatureFlags.ENABLE_OLD_MOTION_LAW_ENGINE) {
            try {
                action()
            } catch (e: Exception) {
     ...
     */
    inline fun <T> withMotionLawGenerator(action: () -> T): T? {
        return if (FeatureFlags.ENABLE_OLD_MOTION_LAW_GENERATOR) {
            try {
                action()
            } catch (e: Exception) {
     ...
     */
    inline fun <T> withDiagnosticsPreflight(action: () -> T): T? {
        return if (FeatureFlags.ENABLE_OLD_DIAGNOSTICS_PREFLIGHT) {
            try {
                action()
            } catch (e: Exception) {
     ...
     */
    inline fun <T> withPerfDiag(action: () -> T): T? {
        return if (FeatureFlags.ENABLE_OLD_PERF_DIAG) {
            try {
                action()
            } catch (e: Exception) {
     ...
     */
    inline fun <T> withCollocationMotionSolver(action: () -> T): T? {
        return if (FeatureFlags.ENABLE_OLD_COLLOCATION_MOTION_SOLVER) {
            try {
                action()
            } catch (e: Exception) {
