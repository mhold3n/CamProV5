...
data class PanelConfiguration(
    ...
    fun calculateDynamicImportance(
        ...
        adjustedWeight *= when (applicationState) {
            ApplicationState.DESIGN_MODE -> when (contentType) {
            ...
            ApplicationState.ANALYSIS_MODE -> when (contentType) {
            ...
            ApplicationState.SIMULATION_MODE -> when (contentType) {
            ...
            ApplicationState.REPORTING_MODE -> when (contentType) {
            ...
            ApplicationState.COLLABORATION_MODE -> when (contentType) {
        ...
        adjustedWeight *= when (workflowType) {
            WorkflowType.DESIGN_WORKFLOW -> when (contentType) {
                ContentType.PARAMETERS -> 1.15f
                ContentType.ANIMATION -> 1.1f
                else -> 1.0f
            }
            WorkflowType.ANALYSIS_WORKFLOW -> when (contentType) {
                ContentType.PLOTS -> 1.2f
                ContentType.DATA -> 1.15f
                else -> 1.0f
            }
            WorkflowType.SIMULATION_WORKFLOW -> when (contentType) {
                ContentType.ANIMATION -> 1.25f
                ContentType.PARAMETERS -> 1.1f
                else -> 1.0f
            }
            WorkflowType.REPORTING_WORKFLOW -> when (contentType) {
    ...
    fun calculatePreferredSize(
        ...
        val finalWidth = if (aspectRatioAdjustedHeight <= availableHeight * maxHeightRatio) {
            calculatedWidth
        } else {
            aspectRatioAdjustedWidth.coerceAtMost(availableWidth * maxWidthRatio)
        }
        
        val finalHeight = if (aspectRatioAdjustedWidth <= availableWidth * maxWidthRatio) {
            calculatedHeight
        } else {
