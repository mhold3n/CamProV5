...
class LayoutManager {
    ...
    fun shouldUseCompactMode(): Boolean {
        return _currentWindowWidth.value < mediumWindowWidth || 
    ...
    fun shouldUseSingleColumn(): Boolean {
        return _currentWindowWidth.value < smallWindowWidth || 
               _currentTemplate.value == LayoutTemplate.COMPACT ||
     ...
     */
    fun shouldEmphasizeDesign(): Boolean {
        return _currentTemplate.value == LayoutTemplate.DESIGN_WORKFLOW
    }
     ...
     */
    fun shouldEmphasizeAnalysis(): Boolean {
        return _currentTemplate.value == LayoutTemplate.ANALYSIS_WORKFLOW
    }
     ...
     */
    fun shouldEmphasizeSimulation(): Boolean {
        return _currentTemplate.value == LayoutTemplate.SIMULATION_WORKFLOW
    }
     ...
     */
    fun shouldEmphasizeReporting(): Boolean {
        return _currentTemplate.value == LayoutTemplate.REPORTING
    }
     ...
     */
    fun shouldEmphasizeCollaboration(): Boolean {
        return _currentTemplate.value == LayoutTemplate.COLLABORATION
    }
     ...
     */
    fun shouldShowDevelopmentInfo(): Boolean {
        return _currentTemplate.value == LayoutTemplate.DEVELOPMENT
    }
     ...
     */
    fun shouldOptimizeForPresentation(): Boolean {
        return _currentTemplate.value == LayoutTemplate.PRESENTATION
    }
    ...
    fun updateApplicationState(newState: ApplicationState) {
        ...
        _currentWorkflowType.value = when (newState) {
        ...
        emitLayout("application_state_changed", mapOf(
    ...
    fun calculateProportionalSizes(
        ...
        for ((panelId, config) in _panelConfigurations.value) {
            ...
            val dynamicImportance = config.calculateDynamicImportance(
        ...
        for ((panelId, config) in _panelConfigurations.value) {
            ...
            val preferredSize = config.calculatePreferredSize(
                effectiveWidth,
                effectiveHeight,
                importance,
    ...
    fun applyWorkflowTemplate(template: WorkflowTemplate) {
        ...
        emitLayout("workflow_template_applied", mapOf(
     ...
     */
    fun getPanelConfiguration(panelId: String): PanelConfiguration? {
        return _panelConfigurations.value[panelId]
    }
    
    /**
     * Get panel usage data by ID
     */
    fun getPanelUsageData(panelId: String): PanelUsageData? {
        return _panelUsageData.value[panelId]
    }
    ...
    fun resetPanelUsageData() {
        val resetData = _panelUsageData.value.mapValues { (panelId, _) ->
 ...
 */
@Composable
fun rememberLayoutManager(): LayoutManager {
    return remember { LayoutManager.getInstance() }
}