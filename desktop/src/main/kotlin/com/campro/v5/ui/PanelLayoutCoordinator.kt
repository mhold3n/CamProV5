...
class PanelLayoutCoordinator {
    ...
    fun registerPanel(id: String, initialState: PanelState) {
        ...
        layoutConstraints[id] = LayoutConstraints(
    ...
    fun enforceSpacing(spacing: Dp) {
        ...
        adjustments.forEach { adjustment ->
            ...
            if (currentState != null) {
                panelStates[adjustment.panelId] = currentState.copy(
                    x = adjustment.newX,
                    y = adjustment.newY,
    ...
    private fun calculateSpacingAdjustment(
        ...
        if (horizontalGap >= 0.dp && horizontalGap < requiredSpacing) {
            val newX = if (panel1Right <= panel2.x) {
        ...
        if (verticalGap >= 0.dp && verticalGap < requiredSpacing) {
            val newY = if (panel1Bottom <= panel2.y) {
 ...
 */
@Composable
fun rememberPanelLayoutCoordinator(): PanelLayoutCoordinator {
    return remember { PanelLayoutCoordinator() }
}