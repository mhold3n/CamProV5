...
class EventSystemTest {
    ...
    fun testEventFiltering() = runBlocking {
        ...
        val receivedClickEvent = withTimeout(1000) {
            clickEvents.first()
        }
        
        val receivedValueChangedEvent = withTimeout(1000) {
    ...
    fun testEventExtensionFunctions() = runBlocking {
        ...
        val receivedTabSelectedEvent = withTimeout(1000) {
            tabSelectedEvents.first()
        }
        
        val receivedGestureEvent = withTimeout(1000) {
            gestureEvents.first()
        }
        
        val receivedErrorEvent = withTimeout(1000) {
            errorEvents.first()
        }
    ...
    fun testEventPerformance() = runBlocking {
        ...
        val job = EventSystem.collectEvents("click") { event ->
            receivedCount++
            latch.countDown()
        }
        
        // Prepare batch of events
        val events = (1..numEvents).map { i ->
