 ...
 */
class MotionLawGeneratorPropertyTest :
    FunSpec({
        ...
        context("Motion Law Properties") {
            ...
            test("C1/C2 continuity at segment boundaries based on profile type") {
                forAll(ciSeed, paramsGen) { params ->
                    ...
                    val fixedBudget =
                        params.dwellTdcDeg + params.dwellBdcDeg +
                    ...
                    val result =
                        keyPoints.all { theta ->
                            ...
                            if (!ok) {
                                println(
                                    "[DEBUG_LOG] continuity fail @theta=$theta profile=${params.rampProfile} step=${motionLaw.stepDeg} rampSpan=$rampSpanDeg u=$u vDiff=$vDiff vTol=$vTolerance aDiff=$aDiff aTol=$aTolerance dBefore=$distBeforeSteps dAfter=$distAfterSteps",
                                )
                                println(
                                    "[DEBUG_LOG] before(theta=${before.thetaDeg}, v=${before.vMmPerOmega}, a=${before.aMmPerOmega2}); after(theta=${after.thetaDeg}, v=${after.vMmPerOmega}, a=${after.aMmPerOmega2})",
                                )
