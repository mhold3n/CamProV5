...
object FixtureLoader {
    fun loadMotionSamples(pathStr: String): FixtureMotionSamples {
        ...
        val existingPath = candidates.firstOrNull { Files.exists(it) }
        ...
        val generatorBlock = Regex("\"generator\"\\s*:\\s*\\{(.*?)\\}", RegexOption.DOT_MATCHES_ALL)
            .find(text)?.groupValues?.get(1)
        val generator = generatorBlock?.let { gb ->
