...
def test_robust_contact_ratio_on_litvin_geometry():
    ...
    # Use varying addendum and dedendum to create realistic variation
    addendum = 2.0 + 0.5 * np.cos(theta)
    dedendum = 2.5 + 0.3 * np.sin(theta)
...
def test_robust_gear_radius_under_litvin_conditions():
    ...
    # Use higher torque values to ensure reasonable gear radii
    torque = 500.0 + 200.0 * np.cos(theta)  # 300-700 N⋅m range
